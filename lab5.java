...
class savacct extends account
{
	...
	void in()
	{
		...
		int t=s.nextInt();
		double x=(1+(r/n));
		double ci=bal*Math.pow(x,(n*t));
		System.out.println("Interest amount="+(ci-bal)+" \nBalance amount without interest is"+bal);
		bal=ci;
...
void wd()
	{
		...
		if(bal<1000)
		{
			System.out.println("You are running out of minimum balance \nAmount of rs 50 will be deducted as service charge for having low balance ");
			System.out.println("Do you want to continue with your transaction with fine?\nPress 1 if yes ");
			int op=s.nextInt();
			if(op==1)
			{
			bal=bal-50;
